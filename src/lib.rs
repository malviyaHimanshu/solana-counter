...
mod test {
    ...
    fn test_counter() {
        ...
        println!("this is account data : {:?}", data);
        ...
        let inc_user_input: u32 = 5u32;
        increment_instruction_data.extend_from_slice(&inc_user_input.to_le_bytes());
        process_instruction(&program_id, &accounts, &increment_instruction_data).unwrap();
        assert_eq!(
            CounterAccount::try_from_slice(&accounts[0].data.borrow()).unwrap().counter,
            5
        );

        let dec_user_input = 5u32;
        decrement_instruction_data.extend_from_slice(&dec_user_input.to_le_bytes());
